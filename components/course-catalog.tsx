  ...
  return <section className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-8 lg:px-12 lg:py-16">
    ...
    {loading ? <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3" aria-label="جارٍ تحميل الدورات">{[1, 2, 3].map((item) => <div key={item} className="h-80 animate-pulse rounded-lg bg-surface-container" />)}</div> : courses.length ? <StaggerGroup className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {courses.map((course) => <StaggerItem key={course.id}>
        <Link href={`/course/${course.id}`} className="group overflow-hidden rounded-lg bg-white shadow-level-2 transition duration-200 hover:-translate-y-1 hover:shadow-level-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">
