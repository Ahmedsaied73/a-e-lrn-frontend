...
export function EnrollmentCard({ courseId, isEnrolled, courseTitle, coursePrice, courseDuration, questionsCount, className }: EnrollmentCardProps) {
  ...
  return <aside className={cn('overflow-hidden rounded-lg bg-white shadow-level-2', className)}>
    ...
    <button onClick={handleEnrollment} disabled={isEnrolled} className={cn('m-5 flex w-[calc(100%-2.5rem)] items-center justify-center rounded-md px-4 py-3 text-sm font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2', isEnrolled ? 'bg-[#e8f2ff] text-primary' : 'bg-primary text-white hover:bg-[#0057c0]')}>
      {isEnrolled ? 'أنت مشترك في الكورس حالياً' : 'اشترك في الدورة'}
    </button>
