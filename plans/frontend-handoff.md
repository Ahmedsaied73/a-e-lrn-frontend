...
**Demo seed (BE `scripts/seedDemoQuizzes.js`, run once):** course #8 videos **ids 4 / 5 / 9** (positions 1/2/3) have quizzes 6/7/8; `seqaccess@localhost.test` holds pre-passed GRADED attempts (gate unlocked); `grader-demo@localhost.test` (password `GraderDemo#2026`) completed v1 and left an **essay attempt in GRADING** — grade it in `/admin/grading` to test the inbox immediately. (Attempt #87 was consumed by P3 checkpoint verification — re-run the seed to re-arm the inbox.)

**Session note:** `POST /auth/register` now preserves an existing session — it sets the new user's login cookies only when the caller has **no** session (`optionalAuth`). This matters for the admin add-student dialog (reuses this public endpoint): before BE `ef97dd8`, registering a student silently replaced the admin's `accessToken`/`refreshToken` cookies and broke every subsequent admin call (403). Public signups behave exactly as before (auto-login via cookies).
